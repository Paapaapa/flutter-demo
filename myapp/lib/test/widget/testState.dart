...
class _CounterWidgetState extends State<CounterWidget> {
  ...
  void initState() {
    ...
    // 所以，通常在该回调中做一些一次性的操作，如状态初始化、
    // 订阅子树的事件通知等。不能在该回调中调用
    // BuildContext.inheritFromWidgetOfExactType
    // （该方法用于在Widget树上获取离当前widget最近的一个父级InheritFromWidget，
    // 关于InheritedWidget我们将在后面章节介绍），
  ...
  void didUpdateWidget(CounterWidget oldWidget) {
    ...
    // Widget.canUpdate来检测Widget树中同一位置的新旧节点，
    // 然后决定是否需要更新，如果Widget.canUpdate返回true
    // 则会调用此回调。正如之前所述，Widget.canUpdate
    // 会在新旧widget的key和runtimeType同时相等时会返回true，
    // 也就是说在在新旧widget的key和runtimeType同时相等时
  ...
  void deactivate() {
    ...
    // 如包含此State对象的子树在树的一个位置移动到另一个位置时
    // （可以通过GlobalKey来实现）。
